...
module.exports = class Help extends baseCmd {
    ...
    async run(msg, args, data){
        if(args[0].toLowerCase() == "set" || args[0].toLowerCase() == "add" || args[0].toLowerCase() == "enable"){
            ...
            let channel = msg.channelMentions[0] ? msg.channel.guild.channels.get(msg.channelMentions[0]) : null || await msg.channel.guild.channels.get(args[1]) || await msg.channel.guild.channels.find(x => x.name.toLowerCase() == args[1].toLowerCase());
            ...
            try{
                ...
                if(err != "DiscordRESTError [10015]: Unknown Webhook") return msg.channel.sendRedEmbed('An error occurred while  the webhook, ' + err);
            }
